...
Target evidence has shown that some Zorin states are image-backed rather than plain `background-color` states. The verified ZorinBlue-Dark GTK 3 base paints generic selected rows with pale cyan and checked switches with `background: image(#bde6fb)`. GoreeCloud therefore maps GTK 3 generic selected-row and checked-switch image states directly to the current variant's Glaze selection/accent tokens. GTK 4 has its own separately validated selected-row and checked-switch mappings for applications that actually use that toolkit path.
