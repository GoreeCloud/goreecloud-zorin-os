...
## Release status
Development / nonconformant. Core maintenance execution is representative-device verified through dev5, and dev6 normal-text compact/minimum-window acceptance is verified. Dev7 passed local source/package validation and exact-head GitHub workflows but failed the representative-device combined `GDK_DPI_SCALE=2` + compact rendering gate: at the narrowest reachable large-text window, the regular HeaderBar/count/action composition remained active. Dev8 remediates that verified defect by evaluating the 820-logical-pixel compact contract against a text-scale-adjusted effective width. Target-device dev8 revalidation, HighContrast, assistive-technology acceptance, and remaining Integral Platform System evidence remain open.
