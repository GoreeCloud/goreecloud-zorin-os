...
class RepresentativeAcceptanceContractTests(unittest.TestCase):
    ...
    def test_preparation_harness_is_non_destructive_and_unprivileged(self) -> None:
        self.assertIn(
            "read-only/non-destructive Stable qualification representative acceptance evidence",
            self.source,
        )
        self.assertIn(
    ...
    def test_preparation_harness_requires_clean_exact_stable_source(self) -> None:
        ...
        self.assertIn("Representative Stable qualification preparation requires lifecycle: stable", self.source)
    ...
    def test_preparation_harness_records_exact_source_and_package_provenance(self) -> None:
        ...
        self.assertIn("lifecycle=stable", self.source)

    def test_preparation_harness_gates_installed_status_on_exact_stable_runtime(self) -> None:
        ...
        self.assertIn("Stable-only status snapshots were skipped", self.source)

    def test_manual_checklist_keeps_stable_delta_and_platform_gates_explicit(self) -> None:
        for required in (
            "Stable identity delta",
            ...
            "Privacy Shield exact-Stable",
            "Wardveil exact-Stable",
            "Everkeep exact-Stable",
            "Glaze V1.2 exact-Stable",
            "V1.3 remains Proposed / consumer-ineligible",
        ):
            self.assertIn(required, self.source)

    def test_preparation_harness_never_self_promotes(self) -> None:
        self.assertIn("Stable qualification representative acceptance preparation: passed", self.source)
        self.assertIn("Stable promotion authorized: false", self.source)
        self.assertNotIn("Stable promotion authorized: true", self.source)
        self.assertNotIn("lifecycle=release-candidate", self.source)
