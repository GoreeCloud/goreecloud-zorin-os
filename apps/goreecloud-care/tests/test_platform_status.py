...
from goreecloud_care.platform_status import API_VERSION, build_continuity_status, build_health_status, build_privacy_status, build_wardveil_status, evaluate_privileged_boundary, render_json
...
class PlatformStatusTests(unittest.TestCase):
    ...
    def test_privileged_boundary_passes_only_with_secure_fixed_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            helper, policy, pkexec = root/"helper", root/"policy", root/"pkexec"
            ...
            checks = evaluate_privileged_boundary(helper, policy, pkexec, expected_uid=os.geteuid())
            self.assertTrue(all(checks.values()))
            payload = build_wardveil_status(NOW, helper_path=helper, policy_path=policy, pkexec_path=pkexec, expected_uid=os.geteuid())
            self.assertEqual(payload["state"], "protected")
            self.assertFalse(payload["claim"]["protected_by_wardveil"])
            self.assertEqual(payload["evidence"]["valid_until"], (NOW + timedelta(minutes=15)).isoformat().replace("+00:00", "Z"))

    def test_privileged_boundary_fails_closed_on_writable_evidence(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            helper, policy, pkexec = root/"helper", root/"policy", root/"pkexec"
    ...
    def _build_provenance(self) -> dict:
        return {"schema_version":1,"application":"GoreeCloud Care","producer":"GoreeCloud/goreecloud-zorin-os/apps/goreecloud-care","source_revision":SOURCE_REVISION,"source_tree":SOURCE_TREE,"runtime_version":RUNTIME_VERSION,"package_version":PACKAGE_VERSION,"source_date_epoch":1789400000,"package_sha256_embedded":False}

    def _acceptance_record(self, *, package_sha256: str = PACKAGE_SHA256, source_revision: str = SOURCE_REVISION, promoted: bool = False) -> dict:
        return {"schema_version":1,"application":"GoreeCloud Care","producer":"GoreeCloud/goreecloud-zorin-os/apps/goreecloud-care","candidate":{"source_revision":source_revision,"source_tree":SOURCE_TREE,"runtime_version":RUNTIME_VERSION,"package_version":PACKAGE_VERSION,"package_sha256":package_sha256},"target":{"name":"Zorin OS 17.3 representative laptop","representative":True,"status":"passed"},"dimensions":["restore_capability","migration","documentation","provenance"],"evidence":{"local_tests":1,"source_validation":"passed","package_lifecycle":"passed","references":["representative acceptance fixture"]},"acceptance":{"target_runtime_status":"passed","exact_revision_accepted":True,"everkeep_integration_promoted":promoted,"everkeep_ready_promoted":promoted,"freshness_rule":"Exact 0.2.0-dev3 source, source tree, package version, package SHA-256, and target only."}}

    def _paths(self, root: Path):
        return root/"package"/"build-provenance.json", root/"representative"/"representative-target.json", root/"everkeep"/"goreecloud-care.target-runtime.json"

    def test_continuity_requires_exact_representative_target_acceptance(self):
        with tempfile.TemporaryDirectory() as tmp:
            provenance, representative, everkeep = self._paths(Path(tmp))
    ...
    def test_continuity_rejects_mismatched_package_and_source(self):
        with tempfile.TemporaryDirectory() as tmp:
            provenance, representative, everkeep = self._paths(Path(tmp))
            self._write_secure_json(provenance, self._build_provenance())
            self._write_secure_json(representative, self._acceptance_record())
            self._write_secure_json(everkeep, self._acceptance_record(package_sha256="d"*64, promoted=True))
            payload = build_continuity_status(NOW, provenance_path=provenance, representative_acceptance_path=representative, everkeep_acceptance_path=everkeep, expected_uid=os.geteuid())
            self.assertEqual(payload["stage"], "target-accepted-governance-pending")
